...
// And an extra space for '\0' for path buffer
  ...
  */
  ...
  bool SDClass::begin(SPIClass &user_spi, uint8_t csPin) {
    if (root.isOpen()) {
      root.close();
    }
    ...
    */
    return card.init(SPI_HALF_SPEED, csPin, user_spi) &&
           volume.init(card) &&
           root.openRoot(volume);
  }

  bool SDClass::begin(SPIClass &user_spi, uint32_t clock, uint8_t csPin) {
    if (root.isOpen()) {
      root.close();
    }
