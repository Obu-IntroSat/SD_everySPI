/*
  ...
  ********************************************************************
  ...
  Now method "begin" allows to choose different SPIs.

*/

#ifndef __SD_EVERYSPI_H__
#define __SD_EVERYSPI_H__

#include <Arduino.h>

#include "utility/SdFat_everySPI.h"
#include "utility/SdFatUtil_everySPI.h"
...
namespace SDLib {
  ...
  class SDClass {
    ...
    public:
      ...
      bool begin(SPIClass &user_spi, uint8_t csPin = SD_CHIP_SELECT_PIN);
      bool begin(SPIClass &user_spi, uint32_t clock, uint8_t csPin);
	  bool begin(uint8_t csPin = SD_CHIP_SELECT_PIN) {
		  return begin(SPI, csPin);
	  }
	  bool begin(uint32_t clock, uint8_t csPin) {
		  return begin(SPI, clock, csPin);
	  }	  
